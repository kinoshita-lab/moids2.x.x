...
#include <digitalWriteFast.h> // must include via library manager
#include <stdint.h>
...
volatile int pulse_high_table[] = {
    125,  125,  250,  125,  250, 125, 250, 250,  375, 500,  625,  500, 250,
    375,  250,  375,  500,  250, 375, 500, 625,  750, 500,  625,  750, 875,
    500,  875,  1000, 500,  625, 625, 875, 1000, 875, 1000, 1250, 875, 1000,
    1125, 1250, 1325, 1450, 0,   0,   0,   0,    0,   0,    0,
};
volatile int *pulse_high = &pulse_high_table[0];

volatile int pulse_low_table[] = {
    125,  250,  250,  375,  375,  500,  875,  1125, 1250, 1250,
    1375, 500,  625,  625,  750,  750,  750,  875,  875,  875,
    875,  875,  1000, 1000, 1000, 1000, 1125, 1125, 1125, 1250,
    1250, 1325, 1325, 1325, 1450, 1325, 1325, 1250, 1250, 1250,
    1325, 1450, 1450, 0,    0,    0,    0,    0,    0,    0,
};
...
volatile int sequence_length_table[] = {
    ...
    0, 1, 0,  1,  0,  1, 0, 1, 3, 3, 3, 60 * 14, 5, 4, 3, 2, 1, 1, 600,
};
...
void setup() {
  ...
  TIMSK0 &= ~_BV(TOIE0); // disable timer0 overflow interrupt(millis() and micros() are not working now)
  pinMode(6, OUTPUT);
  pinMode(9, OUTPUT);
  pinMode(10, OUTPUT);
}
...
void loop() {
  ...
  pinMode(2, OUTPUT);
  while (true) {
    volatile const auto audio_1 = analogRead(0);
    volatile const auto audio_2 = analogRead(0);

    const auto delta = abs(audio_2 - audio_1);
    if (delta > 50) {
      printf("delta: %d\n", delta);
    }
    ...
    if (current_millis - last_millis > 10) {
      ...
      toggle = !toggle;
      digitalWriteFast(2, toggle ? HIGH : LOW);
    }
  }
  return;
  // if (print_sec) {
